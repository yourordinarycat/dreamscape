...
pub enum FrontMatterParseError {
    ...
    InvalidStructure(#[from] yaml_serde::Error),
...
pub fn extract_front_matter(content: &str) -> Result<FrontMatter, FrontMatterParseError> {
    if !content.starts_with("---") {
        return Err(FrontMatterParseError::InvalidFileFormat(
    ...
    if parts.len() >= 3 {
        ...
        yaml_serde::from_str::<FrontMatter>(yaml_block)
            .map_err(FrontMatterParseError::InvalidStructure)
    } else {
        Err(FrontMatterParseError::InvalidFileFormat(
